...
class OC_Bags(data_utils.Dataset):
    ...
    def _create_bags(self):
        ...
        for p in range(len(self.list_paths)):
            ...
            X = images
            all_bags.append(X) 
            if basePath.parts[-1] in self.list_positive_bags:
                y = torch.ones(1) 
            else:
                y = torch.zeros(1) 
            all_bags_labels.append(y) #bags labels, not each instance
...
#             label = [max(self.valid_labels_list[index]), self.valid_labels_list[index]]
            label = [self.valid_labels_list[index]]
...
#             label = [max(self.test_labels_list[index]), self.test_labels_list[index]]
        ...
        if bag.shape[0] > max_number_of_imgs_in_bag: 
            ...
            label_sampled = label #[label[0],label[1][sample_indices]]
