...
from functions_OC import list_files_in_folder, create_save_dir, OC_Bags, AddGaussianNoise, count_max_num_instances, test, test_bags_info
from attention_models import Attention_bags_1GPU
from evaluationOC import compute_metrics
...
train_loader = data_utils.DataLoader(OC_Bags(train=True,
                                               valid=False,
                                               test=False,
...
valid_loader = data_utils.DataLoader(OC_Bags(train=False,
                                              valid=True,
                                              test=False,
                                     transform=transforms.Compose([transforms.RandomHorizontalFlip(p=0.5),
                                                         transforms.RandomVerticalFlip(p=0.5),                                 
...
for epoch in range(1, args.epochs + 1):
    ...
    name_epoch_model = os.path.join(os.path.join(data_path), 'saved_model_OC_'+model_architecture+path.parts[-1]+'_currentEpoch'+\
    ...
    if len(all_valid_error)>window_length-1:
        ...
        if window_with_best_avg > avg_valid_error_window:
            ...
            name_best_inMovAv_val_error_model = os.path.join(os.path.join(data_path), 'saved_model_OC_'+model_architecture+path.parts[-1]+
                                                            '_currentEpoch'+
    ...
    for ep in range(0, epoch-window_length):
        model_nametmp = os.path.join(os.path.join(data_path), 'saved_model_OC_'+model_architecture+path.parts[-1]+'_currentEpoch'+
...
new_name_best_inMovAv_val_error_model = os.path.join(save_weights_dir, 'saved_model_best_movingAvg_OC_'+model_architecture+\
...
for ep in range(1,num_epochs+1):
    model_nametmp = os.path.join(os.path.join(data_path), 'saved_model_OC_'+model_architecture+path.parts[-1]+'_currentEpoch'+str(ep)+'_overallEpochs'+str(num_epochs)+'_lr'+str(lr)+'_sampSize'+str(sampling_size_in_instances)+'.pt')
    if os.path.isfile(model_nametmp):
        os.remove(model_nametmp)

""" Test """              
test_loader = data_utils.DataLoader(OC_Bags(train=False,
                                              valid=False,
                                              test=True,
